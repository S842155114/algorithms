...
public class BoyerMoore {
    ...
    private static void preprocessGoodSuffix(String pattern, int[] bmGs) {
        ...
        for (int i = m - 2; i >= 0; i--) {
            ...
            // 而g如果匹配成功会左移，形成了[g,f]区间，如果此时i进入了此区间，证明上一轮g已经经过了当前i的位置
            // 因为每次进入for循环i都会-1，而f为前n次的i值，所以i - f一定是负值
            // 所以m - 1 + i - f是从右向左的，那么suffix[m - 1 + i - f]一定为已经计算过的值。
            // i - g == 当前准备开始匹配的字符位置 - 上一次成功匹配的字符位置
            // 并且可以利用之前计算出的某个 suffix 值，我们就直接复用那个值，避免进入 else 块的复杂比较。
