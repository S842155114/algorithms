...
public class KMP {
    ...
    private int[] generateNext(String p) {
        ...
        for (int i = 1, j = 0; i < len; i++) {
            ...
            // 后续添加：因为j是从0开始，i从1开始，也就是说，如果此位置字符相同则j++且i进入下个循环也++
            // 就相当于j和i都执行了++操作。经过多轮for循环后，如果j和i的当前字符不同，此时i的位置是不动的，而j
            // 则需要从j之前与i已经匹配的串中，找到与当前i之前的最长匹配串长度，也就是next[j-1]，作为j的新长度newJ，其
            // 代表的是：p[0...newj] = p[i-newj...i]
